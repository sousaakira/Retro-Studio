 ...
 * @param {Object} data - { width, height, tiles, tilesetImagePath, tilesetColumns, collision?, priority? }
 */
export function toTMX(data) {
  const { width, height, tiles = [], tilesetImagePath = 'tileset.png', tilesetColumns = 16, collision = [], priority = [] } = data
  const w = Math.max(1, width || 40)
  const h = Math.max(1, height || 30)
  const tileCount = w * h
  const tileData = Array.from({ length: tileCount }, (_, i) => {
    const v = tiles[i] ?? 0
    return v <= 0 ? -1 : v - 1
  })
  ...
  const csvLines = []
  for (let y = 0; y < h; y++) {
    const row = tileData.slice(y * w, (y + 1) * w).map((v) => String(v)).join(',')
    csvLines.push(row)
  }
...
<map version="1.8" tiledversion="1.8.2" orientation="orthogonal" renderorder="right-down" width="${w}" height="${h}" tilewidth="${TILE_SIZE}" tileheight="${TILE_SIZE}" infinite="0" nextlayerid="4" nextobjectid="1">
...
export function fromTMX(xml) {
  try {
    ...
    return { width, height, tiles, tilesetImagePath, collision, priority }
  } catch (e) {
